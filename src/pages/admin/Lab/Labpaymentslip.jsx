  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.payment === "Yes" && formData.billImage) {
        ...
        const {
          data: { publicUrl: url },
        } = supabase.storage.from("bill_image").getPublicUrl(fileName);
        publicUrl = url;
      }
      ...
      <div className="flex-none bg-white border-b border-gray-200 shrink-0 shadow-sm z-10">
        ...
        {showNotification && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
            <div
              className={`relative w-full max-w-md transform transition-all duration-300 animate-scale-in opacity-100`}
            >
              <div
                className={`rounded-lg shadow-2xl overflow-hidden ${notificationType === "success" ? "bg-gradient-to-br from-green-50 to-green-100 border-2 border-green-200" : "bg-gradient-to-br from-red-50 to-red-100 border-2 border-red-200"} `}
              >
                <div className="p-6 text-center">
                  <div
                    className={`inline-flex items-center justify-center w-16 h-16 rounded-full mb-4 ${notificationType === "success" ? "bg-green-100" : "bg-red-100"}`}
                  >
                  ...
                  <h3
                    className={`text-lg font-bold mb-2 ${notificationType === "success" ? "text-green-800" : "text-red-800"}`}
                  >
                    {notificationType === "success" ? "Success!" : "Error!"}
                  </h3>
                  <p
                    className={`text-sm ${notificationType === "success" ? "text-green-600" : "text-red-600"}`}
                  >
                    {notificationMessage}
                  </p>
                  <div
                    className={`mt-4 h-1 w-full ${notificationType === "success" ? "bg-green-200" : "bg-red-200"}`}
                  >
                    <div
                      className={`h-full ${notificationType === "success" ? "bg-green-500" : "bg-red-500"} animate-progress`}
                    ></div>
                  </div>
                </div>
              </div>
        ...
        <div className="px-3 sm:px-4 pt-2">
          ...
          <div className="grid grid-cols-3 gap-2 mb-3 md:grid-cols-6">
            {[
              ...
              <div
                key={idx}
                className={`p-1.5 sm:p-2 ${s.bg} rounded-lg border ${s.border} shadow-sm transition-all hover:shadow-md`}
              >
                <div className="flex flex-col">
                  <span className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase truncate leading-tight">
                    {s.label}
                  </span>
                  <span
                    className={`text-sm sm:text-base font-black ${s.color} mt-0.5 leading-none`}
                  >
                    {s.val}
                  </span>
                </div>
                        ...
                        <tr key={record.id} className="hover:bg-gray-50">
                          <td className="px-4 py-3 text-sm whitespace-nowrap">
                            <button
                              ...
                              className="px-3 py-1.5 text-white bg-green-600 rounded-lg hover:bg-green-700"
                            >
                              Process
                            </button>
                          </td>
                          ...
                          <td className="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">
                            {record.planned1
                              ? new Date(record.planned1).toLocaleString(
                                  "en-GB",
                                  {
                                    day: "2-digit",
                                    month: "2-digit",
                          ...
                          <td className="px-4 py-3 text-sm whitespace-nowrap">
                            <span
                              className={`px-2 py-1 text-xs font-semibold rounded-full ${record.priority === "High" ? "bg-red-100 text-red-700" : record.priority === "Medium" ? "bg-yellow-100 text-yellow-700" : "bg-green-100 text-green-700"}`}
                            >
                              {record.priority}
                            </span>
                          </td>
                    ...
                    {filteredHistoryPayments.length > 0 ? (
                      filteredHistoryPayments.map((record) => (
                        <tr key={record.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">
                            {record.actual1
                              ? new Date(record.actual1).toLocaleString(
                                  "en-GB",
                                  {
                                    day: "2-digit",
                                    month: "2-digit",
                          ...
                          <td className="px-4 py-3 text-sm">
                            <span
                              className={`px-2 py-1 text-xs font-semibold rounded-full ${record.priority === "High" ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
                            >
                              {record.priority}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-sm">
                            <span
                              className={`px-2 py-1 text-xs font-semibold rounded-full ${record.paymentStatus === "Yes" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                            >
                              {record.paymentStatus === "Yes"
                                ? "Paid"
                                : "Unpaid"}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-sm">
                            <button
                              ...
                              className="flex items-center gap-1 text-green-600 bg-green-50 px-2 py-1 rounded hover:bg-green-100"
                            >
          ...
          <div className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto bg-white rounded-lg shadow-xl">
            <div className="sticky top-0 z-10 flex justify-between items-center p-4 bg-white border-b border-gray-200 md:p-6">
              ...
              <button
                ...
                className="p-1 text-gray-400 hover:text-gray-600 ring-1 ring-gray-200 rounded-full"
              >
                <X className="w-6 h-6" />
              </button>
            </div>
            <div className="p-4 md:p-6">
              <div className="p-4 mb-6 bg-green-50 rounded-lg border border-green-200 grid grid-cols-2 gap-3 text-sm md:grid-cols-3">
                <div className="col-span-2 md:col-span-3 font-semibold text-gray-900 border-b pb-1 mb-1">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select
                    ...
                    className="px-3 py-2 w-full border rounded-lg focus:ring-2 focus:ring-green-500"
                  >
                ...
                {formData.payment === "Yes" && (
                  <div>
                    <label className="block mb-1 text-sm font-medium">
                      Upload Bill (Image or PDF - Optional)
                    </label>
                    <input
                      type="file"
                    ...
                    <label
                      htmlFor="bill-upload"
                      className="flex flex-col items-center justify-center p-6 border-2 border-dashed rounded-lg cursor-pointer hover:bg-green-50"
                    >
                      {imagePreview ? (
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        {imagePreview ? "Click to change" : "Max 5MB"}
                      </p>
                    </label>
                  </div>
                )}
              ...
              <div className="flex gap-3 justify-end mt-6">
                ...
                <button
                  ...
                  disabled={loading}
                  className="px-4 py-2 text-white bg-green-600 rounded-lg disabled:opacity-50"
                >
