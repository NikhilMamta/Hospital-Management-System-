    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        showNotificationPopup(
          `Patient successfully assigned to ${selectedDepartment}`,
          "success",
        );
      }
    } catch (error) {
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5 md:gap-4">
        ...
        <div className="p-3 bg-white rounded-lg border border-gray-200 shadow-sm md:p-4">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xl font-bold text-purple-600 md:text-2xl">
              {stats.ipd}
            </p>
          </div>
        </div>
                    ...
                    {isInitialLoad ? (
                      ...
                      filteredHistoryPatients.map((patient) => (
                        <tr key={patient.id} className="hover:bg-gray-50">
                          <td className="px-4 py-3 whitespace-nowrap">
                            <span
                              className={`inline-flex px-2.5 py-1 text-xs font-medium rounded-full ${getDepartmentBadge(patient.department)}`}
                            >
                              {patient.department}
                            </span>
                          </td>
